...
/**
 * class Gring_Validator_Rule_AfterDateRule
 */
class Gring_Validator_Rule_AfterDateRule extends Gring_Validator_Contract_RuleAbstract {
    /**
     * @var
     */
    ...
    public function validate($data, $propertyName, $item) {
        ...
        /* @var Gring_Validator_Mapping_AfterDate $item */
        $date = $item->getDate();
        if (isset($data[$date])) {
            $date = $data[$date];
        }
        ...
        if (is_string($value)) {
            ...
            if (($dt !== false && !array_sum($dt::getLastErrors())) && strtotime($value) >= strtotime($date)) {
                ...
                if (date('Y-m-d', (int)$value) && (int)$value >= (int)$date) {
                    return $data;
                }
            }
